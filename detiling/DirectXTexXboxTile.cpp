        ...
        const XboxImage& xbox)
    {
        ...
        else
        {
            ...
            for (uint32_t item = 0; item < nimages; ++item)
            {
                ...
                if (!XG_DLL::_LoadScanline(row, img->width, img->pixels, img->rowPitch, img->format))
                    return E_FAIL;
            ...
            if (!XG_DLL::_StoreScanline(xbox.GetPointer() + mip.OffsetBytes, mip.SizeBytes, xbox.GetMetadata().format, tiled, tiledPixels))
                return E_FAIL;
        }
        ...
        const XboxImage& xbox)
    {
        ...
        else
        {
            ...
            for (uint32_t item = 0; item < nimages; ++item)
            {
                ...
                {
                    if (!XG_DLL::_LoadScanline(row, img->width, sptr, img->rowPitch, img->format))
                        return E_FAIL;
        ...
        const XboxImage& xbox)
    {
        ...
        else
        {
            ...
            for (uint32_t z = 0; z < slices; ++z)
            {
                ...
                {
                    if (!XG_DLL::_LoadScanline(row, image.width, rptr, image.rowPitch, image.format))
                        return E_FAIL;
            ...
            for (uint32_t z = 0; z < mip.PaddedDepthOrArraySize; ++z)
            {
                ...
                for (uint32_t y = 0; y < mip.PaddedHeightElements; ++y)
                {
                    ...
                    if (!XG_DLL::_StoreScanline(rptr, mip.PitchBytes, xbox.GetMetadata().format, uptr, mip.PitchPixels))
                        return E_FAIL;
...
#else
        mode = XG_DLL::XGComputeOptimalTileMode(XG_RESOURCE_DIMENSION_TEXTURE2D, static_cast<XG_FORMAT>(srcImage.format),
            static_cast<UINT>(srcImage.width), static_cast<UINT>(srcImage.height),
            1, 1, XG_BIND_SHADER_RESOURCE);
#endif
    }
    ...
    XboxTileMode mode)
{
...
#else
        // If no specific tile mode is given, assume the optimal default
        mode = XG_DLL::XGComputeOptimalTileMode(static_cast<XG_RESOURCE_DIMENSION>(metadata.dimension), static_cast<XG_FORMAT>(metadata.format),
            static_cast<UINT>(metadata.width), static_cast<UINT>(metadata.height),
            static_cast<UINT>((metadata.dimension == TEX_DIMENSION_TEXTURE3D) ? metadata.depth : metadata.arraySize),
            1, XG_BIND_SHADER_RESOURCE);
#endif
    ...
    case TEX_DIMENSION_TEXTURE1D:
    {
        ...
        desc.Usage = XG_USAGE_DEFAULT;
        desc.BindFlags = XG_BIND_SHADER_RESOURCE;
        desc.MiscFlags = (metadata.IsCubemap()) ? XG_RESOURCE_MISC_TEXTURECUBE : (XG_RESOURCE_MISC_FLAG)0;
#if defined(_GAMING_XBOX_SCARLETT) || defined(_USE_SCARLETT)
        desc.SwizzleMode = mode;
#else
        desc.TileMode = mode;
    ...
    case TEX_DIMENSION_TEXTURE2D:
    {
        ...
        desc.Usage = XG_USAGE_DEFAULT;
        desc.BindFlags = XG_BIND_SHADER_RESOURCE;
        desc.MiscFlags = (metadata.miscFlags & TEX_MISC_TEXTURECUBE) ? XG_RESOURCE_MISC_TEXTURECUBE : (XG_RESOURCE_MISC_FLAG)0;
#if defined(_GAMING_XBOX_SCARLETT) || defined(_USE_SCARLETT)
        desc.SwizzleMode = mode;
#else
        desc.TileMode = mode;
